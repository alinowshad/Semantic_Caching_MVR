...
int main(int argc, char **argv) {
    ...
    for (int k : k_values) {
        ...
        for (int i = 0; i < NQ_chunks; i++) {
            ...
            std::unordered_map<hnswlib::labeltype, std::vector<hnswlib::labeltype>> parent_to_vec_ids;
            for (size_t j = 0; j < query_chunk_size; ++j) {
                ...
                while (!initial_results.empty()) {
                    ...
                    parent_to_vec_ids[parent_id].push_back(vec_id);
                }
            }
            // Stage 2: Re-rank candidate parent documents using MinDist
            // std::cout << "====================" << std::endl;
            // std::cout << "checking the parent ids" << std::endl;
            std::vector<std::pair<float, hnswlib::labeltype>> reranked_parents;
            reranked_parents.reserve(candidate_parent_ids_set.size());
            for (const auto& parent_id : candidate_parent_ids_set) {
                ...
                float distance = MinDistSameParent(query_vecs, data_vecs, dim,
                    query_chunk_size, query_offset, parent_to_vec_ids[parent_id], &space);

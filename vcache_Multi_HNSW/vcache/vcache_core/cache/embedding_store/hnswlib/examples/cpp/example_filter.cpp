...
int main() {
    ...
    for (int i = 0; i < max_elements; i++) {
        alg_hnsw->addPoint(data + i * dim, i, i);
    }
    ...
    int k = 10;
    for (int i = 0; i < max_elements; i++) {
        std::vector<std::tuple<float, hnswlib::labeltype, hnswlib::labeltype>> result = alg_hnsw->searchKnnCloserFirst(data + i * dim, k, &pickIdsDivisibleByTwo);
        for (auto item: result) {
            if (std::get<1>(item) % 2 == 1) std::cout << "Error: found odd label\n";
        }
    }

...
class AlgorithmInterface {
 public:
    virtual void addPoint(const void *datapoint, labeltype label, labeltype parent_id, bool replace_deleted = false) = 0;

    virtual std::priority_queue<std::tuple<dist_t, labeltype, labeltype>>
        searchKnn(const void*, size_t, BaseFilterFunctor* isIdAllowed = nullptr) const = 0;

    // Return k nearest neighbor in the order of closer fist
    virtual std::vector<std::tuple<dist_t, labeltype, labeltype>>
...
template<typename dist_t>
std::vector<std::tuple<dist_t, labeltype, labeltype>>
AlgorithmInterface<dist_t>::searchKnnCloserFirst(const void* query_data, size_t k,
                                                 BaseFilterFunctor* isIdAllowed) const {
    std::vector<std::tuple<dist_t, labeltype, labeltype>> result;

...
int main(int argc, char **argv) {
    ...
    std::string index_path = "/data/ali/" + std::string(DATASET) + "_index_raw.bin";
    ...
    // FunctionPool func_pool(1024); // preallocate 1024 reusable slots
    ...
    for (int k : k_values) {
        ...
        for (int i = 0; i < NQ_chunks; i++) {
            ...
            for (size_t j = 0; j < query_chunk_size; ++j) {
                pool.enqueue([&, j] {
                    auto results = alg_hnsw->searchKnn(query_vecs.row(query_offset + j).data(), k);

...
class VerifiedSplitterDecisionPolicy(VCachePolicy):
    ...
    def _maxsim_similarity(self, query: str, candidate: str) -> float:
        ...
        # Default slow-ish path: encode both strings inside the splitter.
        # The optimized request path encodes the query once and reuses it; see `_maxsim_similarity_from_encoded`.
        query_tensor, corpus_tensor = self.splitter.split_pair_return_maxsim_tensors(query, candidate)

        # Use the tensor's device for all math

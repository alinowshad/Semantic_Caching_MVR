...
int main() {
    ...
    for (int i = 0; i < max_elements; i++) {
        ...
        alg_hnsw->addPoint(point_data, label, label);
        alg_brute->addPoint(point_data, label, label);
    }
    ...
    for (int i = 0; i < num_queries; i++) {
        ...
        std::priority_queue<std::tuple<float, hnswlib::labeltype, hnswlib::labeltype>> result_brute =
            alg_brute->searchKnn(query_data, max_elements);
        
        // check recall
        std::unordered_set<hnswlib::labeltype> gt_labels;
        while (!result_brute.empty()) {
            float dist = std::get<0>(result_brute.top());
            hnswlib::labeltype label = std::get<1>(result_brute.top());
            if (dist < epsilon2) {

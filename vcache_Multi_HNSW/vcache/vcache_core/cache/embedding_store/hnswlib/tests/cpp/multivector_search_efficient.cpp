...
int main(int argc, char **argv) {
    ...
    for (int k : k_values) {
        ...
        for (int i = 0; i < NQ_chunks; i++) {
            ...
            std::unordered_map<hnswlib::labeltype, std::vector<hnswlib::labeltype>> candidate_same_parent_ids;
            
            size_t current_k = k;
            while (candidate_parent_ids.size() < k) {
                ...
                for (size_t j = 0; j < query_chunk_size; ++j) {
                    auto initial_results = alg_hnsw->searchKnnSameParent(query_vecs.row(query_offset + j).data(), current_k);
                    while (!initial_results.empty()) {
                        auto t = initial_results.top();
                        auto id = std::get<1>(t);
                        auto parent = std::get<2>(t);
                        const auto &same_labels = std::get<3>(t);
                        // std::cout << "Parent " << parent << " id " << id
                        //           << ": same-parent neighbor count = " << same_labels.size() << ", ids = [";
                        // for (size_t idx = 0; idx < same_labels.size(); ++idx) {
                        //     if (idx) std::cout << ",";
                        //     std::cout << same_labels[idx];
                        // }
                        // std::cout << "]\n";
                        candidate_parent_ids.insert(parent);
                        candidate_same_parent_ids[parent] = same_labels;
                        initial_results.pop();
                    }
                }

                if (candidate_parent_ids.size() >= k) {
                    break;
                }
                current_k *= 2;
            }
            ...
            for (const auto& parent_id : candidate_parent_ids) {
                ...
                // std::cout << "candidate_same_parent_ids[parent_id].size() = " << candidate_same_parent_ids[parent_id].size() << std::endl;

                if (candidate_same_parent_ids[parent_id].size() > 0 && candidate_same_parent_ids[parent_id].size() >= data_chunk_size) {
                    float distance = MinDistSameParent(query_vecs, data_vecs, dim,
                    query_chunk_size, query_offset, candidate_same_parent_ids[parent_id], &space);
                ...
                // std::cout << "====================" << std::endl;
                // std::cout << "original parent_id = " << parent_id << std::endl;
                // std::cout << "chunks size = " << data_chunk_size << std::endl;
                // std::cout << "same parent size = " << candidate_same_parent_ids[parent_id].size() << std::endl;
                // std::cout << "====================" << std::endl;
                // float distance = MinDist(query_vecs, data_vecs, dim,
                //     query_chunk_size, data_chunk_size,
                //     query_offset, data_offset, &space);

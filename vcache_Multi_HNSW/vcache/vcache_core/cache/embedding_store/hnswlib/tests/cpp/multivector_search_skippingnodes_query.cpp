...
int main(int argc, char **argv) {
    ...
    per_query_output << "query_parent_id,query_vector_local_idx,query_vector_global_idx,k,predicted_parent_ids" << std::endl;

    // Open the per-query results file for the skipping-duplicates loop
    std::ofstream per_query_output_skipping(per_query_file_skipping);
    if (!per_query_output_skipping.is_open()) {
        std::cerr << "Failed to open per-query (skipping) result file: " << per_query_file_skipping << std::endl;
        return 1;
    }
    per_query_output_skipping << "query_parent_id,query_vector_local_idx,query_vector_global_idx,k,predicted_parent_ids" << std::endl;
    ...
    for (int k : k_values) {
        ...
        double total_searchKnn_time = 0.0;
        double total_searchKnnSkippingDuplicates_time = 0.0;
        ...
        for (int i = 0; i < NQ_chunks; i++) {
            ...
            std::priority_queue<std::pair<float, hnswlib::labeltype>, std::vector<std::pair<float, hnswlib::labeltype>>, hnswlib::HierarchicalNSW<float>::CompareByFirst> tls;
            for (size_t j = 0; j < query_chunk_size; ++j) {
                std::ostringstream vector_oss;
                size_t query_offset = query_parent_info[i].first;
                size_t query_chunk_size = query_parent_info[i].second;

                alg_hnsw->searchKnnSkippingDuplicatesCluster(query_vecs.row(query_offset + j).data(), k, tls);
            }
            while (!tls.empty()) {
                auto t = tls.top();
                tls.pop();
                candidate_parent_ids_set.insert(t.second);
            }
            ...
            // Removed: per-whole-query row. We now write per sub-vector above.
        }
        ...
        // Calculate and display average timing for searchKnn
        std::cout << "k-NN Document Recall@" << k << ": " << recall_knn << ", QPS: " << qps << "\n";
        result_output << k << "," << recall_knn << "," << qps << std::endl;
    }

    result_output << "Per-vector Skipping Vector Strategy" << std::endl;
    ...
    for (int k : k_values) {
        ...
        for (int i = 0; i < NQ_chunks; i++) {
            ...
            for (size_t j = 0; j < query_chunk_size; ++j) {
                ...
                for (size_t r = 0; r < candidate_parent_ids.size(); ++r) {
                    if (r) vector_oss << ';';
                    vector_oss << candidate_parent_ids[r];
                }
                per_query_output_skipping << i << "," << j << "," << k << "," << vector_oss.str() << std::endl;
            }
        ...
        // Calculate and display average timing for searchKnnSkippingDuplicates
        std::cout << "k-NN Document Recall@" << k << ": " << recall_knn << ", QPS: " << qps << "\n";
        result_output << k << "," << recall_knn << "," << qps << std::endl;
    }
    ...
    per_query_output_skipping.close();

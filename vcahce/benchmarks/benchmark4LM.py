...
$env:SPLITTER_DEBUG="1"
$env:SPLITTER_DEBUG_LOG_PATH="..\tmp\splitter_debug.jsonl"
$env:SPLITTER_DEBUG_MAX_RECORDS="200"   # 最多写200条
$env:SPLITTER_DEBUG_EVERY_N="1"
...
logging.basicConfig(
    filename=os.path.join(results_dir, "benchmark4LM.log"),
    level=logging.INFO,
...
SPLITTER_USE_CACHED_CANDIDATE_SEGMENTS: bool = (
    os.environ.get("SPLITTER_USE_CACHED_CANDIDATE_SEGMENTS", "0").strip() in {"1", "true", "True"}
)

SPLITTER_CANDIDATE_SELECTIONS = ["top_k"]
SPLITTER_CANDIDATE_KS = [10]
...
class Benchmark(unittest.TestCase):
    ...
    def run_benchmark_loop_custom(self, data_entries: List[Dict], max_samples: int):
        ...
        for idx, data_entry in enumerate(data_entries):
            ...
            # 1) Get Data
            prompt: str = _clean_prompt_text(data_entry["prompt"])

...
test_approx(std::vector<float> &queries, size_t qsize, hnswlib::HierarchicalNSW<d_type> &appr_alg, size_t vecdim,
    ...
    for (int i = 0; i < qsize; i++) {
        std::priority_queue<std::tuple<d_type, hnswlib::labeltype, hnswlib::labeltype>> result = appr_alg.searchKnn((char *)(queries.data() + vecdim * i), K);
        total += K;
        while (result.size()) {
            if (answers[i].find(std::get<1>(result.top())) != answers[i].end()) {
                correct++;
            } else {
            }
            result.pop();
...
int main(int argc, char **argv) {
    ...
    appr_alg.addPoint((void *)dummy_batch.data(), (size_t)0, (size_t)0);
    ...
    if (update) {
        ...
        ParallelFor(1, N, num_threads, [&](size_t i, size_t threadId) {
            appr_alg.addPoint((void *)(dummy_batch.data() + i * d), i, i);
        });
    ...
    stopw.reset();
    ParallelFor(0, N, num_threads, [&](size_t i, size_t threadId) {
                    appr_alg.addPoint((void *)(final_batch.data() + i * d), i, i);
                });

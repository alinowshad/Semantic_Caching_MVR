...
void test_some_filtering(hnswlib::BaseFilterFunctor& filter_func, size_t div_num, size_t label_id_start) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        alg_brute->addPoint(data.data() + d * i, label_id_start + i, label_id_start + i);
        alg_hnsw->addPoint(data.data() + d * i, label_id_start + i, label_id_start + i);
    }
    ...
    for (size_t j = 0; j < nq; ++j) {
        ...
        while (!gd.empty()) {
            assert(gd.top() == res[--t]);
            assert((std::get<1>(gd.top()) % div_num) == 0);
            gd.pop();
        }
    }

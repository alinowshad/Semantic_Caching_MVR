...
int main(int argc, char **argv) {
    ...
    sprintf(result_file, "/home/ali/hnsw-skipping-construction/results/%s_hnsw.csv", DATASET);
    sprintf(per_query_file, "/home/ali/hnsw-skipping-construction/results/%s_hnsw_per_query.csv", DATASET);
    ...
    float epsilon = 0.1f;  // Small epsilon for upper bound computation
    ...
    for (int k : k_values) {
        ...
        for (int i = 0; i < NQ_chunks; i++) {
            ...
            // std::priority_queue<std::pair<float, hnswlib::labeltype>, 
            //             std::vector<std::pair<float, hnswlib::labeltype>>,
            //             CompareByScore> lower_bound_top_candidates;
            // // Create a temporary copy to iterate over the priority queue
            ...
            //     float score = 0.0f;
            //     for (size_t j = 0; j < query_chunk_size; ++j) {
            //         if (approx_dist_to_parent[candidate.second][j].empty()) {
            //             upper_bound_score += kth_distances[j] + epsilon;
            //             lower_bound_score += 1;
            //         } else {
            //             float best_dist = approx_dist_to_parent[candidate.second][j].back();
            //             upper_bound_score += best_dist;
            //             lower_bound_score += best_dist;
            //         }
            //     }
            //     lower_bound_top_candidates.emplace(lower_bound_score, candidate.second);
            // }

            // float minimum_lower_bound = lower_bound_top_candidates.top().first;

            // std::priority_queue<std::pair<float, hnswlib::labeltype>, 
            // std::vector<std::pair<float, hnswlib::labeltype>>,
            // CompareByScore> filtered_top_candidates;

            // // Create another temporary copy to iterate over the priority queue
            // auto temp_candidates2 = top_candidates;
            // while (!temp_candidates2.empty()) {
            //     auto candidate = temp_candidates2.top();
            //     temp_candidates2.pop();
                
            //     if (candidate.first <= minimum_lower_bound) {
            //         filtered_top_candidates.emplace(candidate.first, candidate.second);
            //     }
            // }

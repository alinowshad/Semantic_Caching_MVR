...
void get_gt(
    ...
    L2Space &l2space,
    size_t vecdim,
    vector<std::priority_queue<std::tuple<float, labeltype, labeltype>>> &answers,
    size_t k) {
    BruteforceSearch<float> bs(&l2space, vecsize);
    for (int i = 0; i < vecsize; i++) {
        bs.addPoint((void *) (mass + vecdim * i), (size_t) i, (size_t) i);
    }
    (vector<std::priority_queue<std::tuple<float, labeltype, labeltype>>>(qsize)).swap(answers);
    //answers.swap(vector<std::priority_queue< std::pair< float, labeltype >>>(qsize));
    for (int i = 0; i < qsize; i++) {
        std::priority_queue<std::tuple<float, labeltype, labeltype>> gt = bs.searchKnn(massQ + vecdim * i, 10);
        answers[i] = gt;
    }
}
...
//#pragma omp parallel for
    for (int i = 0; i < qsize; i++) {
        std::priority_queue<std::tuple<float, labeltype, labeltype>> result = appr_alg.searchKnn(massQ + vecdim * i, 10);
        std::priority_queue<std::tuple<float, labeltype, labeltype>> gt(answers[i]);
        unordered_set<labeltype> g;
        total += gt.size();
        while (gt.size()) {
            g.insert(std::get<1>(gt.top()));
            gt.pop();
        }
        while (result.size()) {
            if (g.find(std::get<1>(result.top())) != g.end())
                correct++;
            result.pop();
        }
    }
...
//#define LOAD_I
    ...
    StopW stopwb = StopW();
    for (int i = 0; i < 1; i++) {
        appr_alg.addPoint((void *) (mass + vecdim * i), (size_t) i, (size_t) i);
    }
#pragma omp parallel for

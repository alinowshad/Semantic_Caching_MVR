...
test_approx(
    ...
    for (int i = 0; i < qsize; i++) {
        std::priority_queue<std::tuple<int, labeltype, labeltype>> result = appr_alg.searchKnn(massQ + vecdim * i, k);
...
void sift_test1B() {
    ...
    if (exists_test(path_index)) {
        ...
        appr_alg->addPoint((void *) (massb), (size_t) 0, (size_t) 0);
        int j1 = 0;
        StopW stopw = StopW();
...
#pragma omp critical
            ...
            appr_alg->addPoint((void *) (mass), (size_t) j2, (size_t) j2);
        }
        input.close();

...
int main(int argc, char **argv) {
    ...
    result_output << "k,Recall,Precision,QPS" << std::endl;
    ...
    per_query_output << "QueryID,k,Recall,Precision" << std::endl;
    StopW stopw;
    ...
    std::vector<float> recall_sums(k_values.size(), 0.0f);
    std::vector<float> precision_sums(k_values.size(), 0.0f);
    for (int i = 0; i < NQ_chunks; i++) {
        ...
        for (size_t idx = 0; idx < k_values.size(); ++idx) {
            ...
            // Declare sets for ground truth and predicted parent IDs
            std::unordered_set<hnswlib::labeltype> gt_parent_ids;
            std::unordered_set<hnswlib::labeltype> predicted_parent_ids;
            
            // Populate ground truth parent IDs
            for (int g = 0; g < gt_vecs.cols(); ++g) {
                hnswlib::labeltype gt_id = gt_vecs(i, g);
                if (gt_id == UINT_MAX) break; // optional: stop if padding
                gt_parent_ids.insert(gt_id);
            }
            // Populate predicted parent IDs from top-k candidates
            for (size_t j = 0; j < upto; ++j) {
                predicted_parent_ids.insert(candidates[j].second);
            }
            
            // Calculate intersection between predicted and ground truth
            int intersection_size = 0;
            for (const auto& pred_id : predicted_parent_ids) {
                if (gt_parent_ids.count(pred_id)) {
                    intersection_size++;
                }
            }
            
            float recall_for_this_query = static_cast<float>(intersection_size) / gt_parent_ids.size();
            float precision_for_this_query = static_cast<float>(intersection_size) / upto;

            recall_sums[idx] += recall_for_this_query;
            precision_sums[idx] += precision_for_this_query;
            
            // Write per-query results to CSV
            per_query_output << i << "," << k << "," << recall_for_this_query << "," << precision_for_this_query << std::endl;
        }
    }
    ...
    for (size_t idx = 0; idx < k_values.size(); ++idx) {
        int k = k_values[idx];
        float recall_knn = recall_sums[idx] / static_cast<float>(NQ_chunks);
        float precision_knn = precision_sums[idx] / static_cast<float>(NQ_chunks);
        std::cout << "k-NN Document Recall@" << k << ": " << recall_knn << ", Precision@" << k << ": " << precision_knn << ", QPS: " << qps << "\n";
        result_output << k << "," << recall_knn << "," << precision_knn << "," << qps << std::endl;
    }

    result_output.close();
    per_query_output.close();

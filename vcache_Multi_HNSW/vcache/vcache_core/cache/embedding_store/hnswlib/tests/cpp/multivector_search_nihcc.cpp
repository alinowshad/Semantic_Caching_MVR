...
int main(int argc, char **argv) {
    ...
    per_query_output << "QueryID,k,Recall,Hit,Precision,QueryTimeUs" << std::endl;
    ...
    for (int k : k_values) {
        ...
        for (int i = 0; i < NQ_chunks; i++) {
            ...
            if (valid_gt > 0 && num_predicted > 0) {
                float recall_for_this_query = static_cast<float>(correct_matches) / valid_gt;
                float hit_for_this_query = (correct_matches > 0) ? 1.0f : 0.0f;
                float precision_for_this_query = static_cast<float>(correct_matches) / num_predicted;
                
                recall_sum += recall_for_this_query;            // Recall@k
                if (correct_matches > 0) hit_sum += 1.0f;                                // Hit@k
                precision_sum += precision_for_this_query;    // Precision@k
                
                // Write per-query results to CSV
                per_query_output << i << "," << k << "," << recall_for_this_query << "," 
                                << hit_for_this_query << "," << precision_for_this_query << "," 
                                << query_time_us << std::endl;
            }
